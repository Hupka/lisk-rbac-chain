...
import { ModalSubmitTransaction } from '../Modals/ModalSubmitTransaction';
...
export default function ConfigurePolicies() {
	...
	const submitInputAsTransaction = async event => {
		event.preventDefault();
		...
		try {
			const txObj = await generateTransaction(
				txData,
				cookies['api-url'] ? cookies['api-url'] : 'http://localhost:4000',
			);
			...
			setRespMsg(duder as Record<string, unknown>);
		} catch (error) {
			setRespMsg(error);
		}
				...
				<div className="bg-white shadow sm:rounded-lg max-w-3xl">
							...
							<div>
								...
								<div className="mt-2 max-w-xl text-sm text-gray-500">
									<p>
										...
										<span className=" italic ">role-based access controls</span> happens by
					...
					<div className="flex flex-col items-center">
						<div className="w-full pb-2 px-6">
							...
							>
								<pre className="overflow font-mono">{example}</pre>
							</div>
